...
A static, data-driven marketplace site: a landing page listing every skill and a per-skill detail page (description, install command, example usage, link to the skill's own repo). It renders client-side via the vendored **DC runtime** (`support.js`, which pulls React/ReactDOM/Babel from the unpkg CDN at load time and mounts the `<x-dc>` template) over the imported **Modernist** design system in `docs/_ds/` — so it needs no build step and works from any static host; publish it by pointing GitHub Pages at the `/docs` folder. `_ds/` and `support.js` are imported artifacts — treat them as vendored and edit only `Landing.dc.html`, `Detail.dc.html`, and `skills-data.js`. **The listing is driven by `docs/skills-data.js`, which must stay in sync with `marketplace.json`'s `plugins[]`** — when a plugin is added, removed, or renamed in the manifest, mirror it in `skills-data.js` (`id`, `name`, `repo`, and the copy fields). There is no `access`/read-write label — each skill's read vs. write behavior is communicated through its description copy, not a badge. The detail page also shows a **Changelog** section: `skills-data.js` mirrors each skill's **latest** `CHANGELOG.md` entry in a `changelog` field (`version`, `date`, and typed `changes[]`), with a "Full changelog on GitHub" link to the skill repo's `CHANGELOG.md` — so when a skill releases a new version, update that skill's `changelog` entry here to match (the metadata validator does not yet cover this mirror). A CI check, `scripts/validate-metadata.mjs`, enforces that the Claude manifest (`.claude-plugin/marketplace.json`), the **Codex manifest** (`.agents/plugins/marketplace.json` — see below), `skills-data.js`, and the pages' static `#fallback` lists all agree on the plugin set and each plugin's `repo` (and install command) — it runs on every pull request (`.github/workflows/validate-metadata.yml`), so this drift fails the build instead of shipping silently. Run it locally with `node scripts/validate-metadata.mjs`.

Because the whole site depends on JavaScript and a CDN fetch, both pages carry a **resilience layer** so they're never blank: a `#boot-status` loading line, a `<noscript>` CSS switch, and a small watchdog that reveals a self-contained static `#fallback` (plugin names, descriptions, install commands, and repo links) if the runtime never mounts `#dc-root` (React CDN blocked, boot error). These elements live **outside** the `x-dc` template so the runtime ignores them, and the fallback paints its own opaque surface so it reads in any theme. Two constraints when editing the pages: **never write the literal `<x-dc>`/`</x-dc>` string anywhere outside the real template element** (even in a comment or CSS) — `support.js` re-parses the raw page source with a `/<x-dc…>/` regex and a stray occurrence hijacks the template; and **keep the static `#fallback` skill list in sync** with `skills-data.js` when plugins change (the metadata check above enforces this — each fallback must list every plugin's repo link and install command, and link no others).

## The Codex marketplace mirror (OpenAI compatibility)

OpenAI's **Codex CLI** has a repo-based plugin marketplace nearly identical to Claude Code's, consuming the same `SKILL.md` format. This repo therefore doubles as a Codex marketplace via `.agents/plugins/marketplace.json` (`name`, `interface.displayName`, `plugins[]`). It is a **derived mirror of `.claude-plugin/marketplace.json`, never a hand-maintained parallel**: same plugin set, and each entry uses a git-backed source (`{ "source": "url", "url": "https://github.com/<owner>/<repo>.git", "ref": "main" }`) that must normalize to the same `<owner>/<repo>` as the Claude manifest, plus Codex-required `policy.installation`, `policy.authentication`, and `category` fields. `validate-metadata.mjs` enforces this mirror, so when a plugin is added/removed/renamed or repointed in the Claude manifest, mirror it here in the same PR. Installed with `codex plugin marketplace add HappypsychoX/Skills-Marketplace --ref main`. **Caveat:** the entries only resolve once each per-skill repo carries a root `.codex-plugin/plugin.json` — that lives in the skill repos, not here. Full investigation and per-surface portability notes: [`docs/openai-compatibility.md`](docs/openai-compatibility.md).
